...
import { checkCamps, checkEmail, checkInvalidLetters, comparePassword} from "../helpers/RegisterHelpers"
...
export async function registerData(userData: IUserRegister){
    ...
    const invalidLetters = checkInvalidLetters(userData.firstName, userData.lastName, userData.userName)
    if(invalidLetters){
        return {
            error: true,
            message: 'Invalid letters in one of your name, lastname or username',
            user: null
        }
    }
    const passwordsCompared = comparePassword(userData.password, userData.passwordCheck)
